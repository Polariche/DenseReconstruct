...
class WarpField:
    ...
    def array(self):
        # output doublequaternion & weights to an array
        n = self.nodes_pos.shape[1]

        return np.concatenate([[*self.nodes_dq[i].array(), self.nodes_weight[i]] for i in range(n)])

    def encodeArray(self, x):
        n = self.nodes_pos.shape[1]

        for i in range(n):
            self.nodes_dq[i].encodeArray(x[i*9:i*9+8])
            self.nodes_weight[i] = x[i*9+8]

    def get_warp(self, X, k=None, differential = False):
        # input should be (3,n)
        X = X.reshape(3,-1)
    ...
    def residual1(self, X, k=None):
        ...
        qh, dqh_dq, ind, dq_list, dq_w = self.get_warp(X, k, differential = True)
        ...
        # compute normals
        # TODO : compute from voxels instead of pixels
        N = frame1.cam2world(frame1.normal(X), rotate_only=True)   # pixelnormals 
...
if __name__ == "__main__":
    ...
    while not converged:
        loss, jacobian, hessian = wf.residual1(frame0.cam2world(frame0.pixel_3d_cam()[:,:10000]))
        v += hessian.dot(jacobian.T).dot(loss)
        wf.encodeArray(v)


        if i>0:
            lossdiff = np.abs(np.mean(loss - old_loss)) 
            converged = lossdiff < 1e-6 or i > 50
            print(lossdiff)
